...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "try {
      ...
      if ($j.balance) {
          $value = [math]::Round(($j.balance/1000000),6);
          Write-Host ('💰 الرصيد المالي الحالي (WorkHub): ' + $value + ' XRP') -ForegroundColor Green
      } elseif ($j.account_data -and $j.account_data.Balance) {
          $value = [math]::Round(($j.account_data.Balance/1000000),6);
          Write-Host ('💰 الرصيد المالي الحالي (WorkHub): ' + $value + ' XRP') -ForegroundColor Green
      } else {
          Write-Host 'تعذر العثور على بيانات الرصيد في الاستجابة.' -ForegroundColor Red
      }
  } catch {
      Write-Host '⚠️ خطأ أثناء الاتصال بالمصدر أو الشبكة: ' + $_.Exception.Message -ForegroundColor Red
  }"
echo.
pause
goto menu
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "try {
      $url = 'https://api.xrpscan.com/api/v1/account/%XRPL_ACCOUNT%/transactions';
      $j = Invoke-RestMethod -Uri $url;
      if ($j.transactions) {
          $i=0;
          foreach ($t in $j.transactions | Select-Object -First 10) {
              $i++;
              $time = if ($t.date) { (Get-Date $t.date -Format 'yyyy-MM-dd HH:mm') } else { '-' };
              $type = if ($t.type) {
                          switch ($t.type) {
                              'Payment' { 'تحويل مالي' }
                              'OfferCreate' { 'إنشاء عرض' }
                              'OfferCancel' { 'إلغاء عرض' }
                              default { $t.type }
                          }
                      } else { 'N/A' };
              $amt = if ($t.amount) { [math]::Round(($t.amount/1000000),6) } else { '-' };
              Write-Host ($i.ToString() + '. ' + $type + ' | ' + $amt + ' XRP | ' + $time) -ForegroundColor Cyan
          }
      } else {
          Write-Host 'لا توجد سجلات متاحة أو المسار غير مدعوم.' -ForegroundColor Yellow
      }
  } catch {
      Write-Host '⚠️ خطأ أثناء جلب سجلات المعاملات: ' + $_.Exception.Message -ForegroundColor Red
  }"
echo.
pause
goto menu
